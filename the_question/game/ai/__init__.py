...
INFERENCE_HOST = "http://localhost:8080"
...
def message(character_profile: str, prompt_messages_list: list[str], user_input: str, messages: list[dict], possible_images: list[str], possible_labels: list[str]) -> tuple[Optional[str], list[dict], Optional[str]]:
    ...
    Args:
        prompt_messages_list (list[str]): The list of messages between the player and the visual novel character.
    ...
    if not prompt_messages_list or not isinstance(prompt_messages_list, list):
        raise TypeError("prompt_messages_list must be a list")
    ...
    messages.extend([
        {
            "role": "assistant",
            "content": (" ".join(prompt_messages_list))
        },
        {
            "role": "user",
            "content": user_input
